...
B<parprouted> [B<--addressless -a>] [B<--debug -d>] [B<--foreground -f>] [B<--permanent -p>] [B<--pidfile -P> I<file>] [B<--sync -s>] B<interface> [B<interface ...>]
...
bridging on, unless you use addressless mode (B<-a>). There is no need for this 
...
B<-a> or B<--addressless>, which enables addressless mode for use when bridged 
interfaces have no IP addresses. This mode implies B<-p> (permanent ARP entries).
...
B<-P> I<file> or B<--pidfile> I<file>, which specifies the location of the PID file.
When specified, the daemon will write its process ID to this file. If not specified,
the default location I</var/run/parprouted.pid> is used when running as a daemon.
No PID file is created when running in foreground mode (B<-f>) unless explicitly
specified with B<-P>.

B<-s> or B<--sync>, which enables IP address synchronization across interfaces.
